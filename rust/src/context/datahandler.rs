...
impl DataHandler {
  ...
  pub fn deg_to_bearing(&self, mut wind_direction: i64) -> &'static str {
    wind_direction = if !(0..=360).contains(&wind_direction) {
      0
    } else {
      wind_direction
    };
  ...
  pub fn get_condition(&self, _cond: &str, _night: bool) -> char {
    return if _night {
      if NIGHTTIME_CODES.contains_key(_cond) {
        *NIGHTTIME_CODES.get(_cond).unwrap()
      } else {
        'c'
      }
    } else if DAYTIME_CODES.contains_key(_cond) {
      *DAYTIME_CODES.get(_cond).unwrap()
    } else {
      'c'
    };
  }
